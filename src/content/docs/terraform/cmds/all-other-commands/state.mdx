...
<Tabs>
    ...
    workspace にリソースが管理されていると
    ```bash frame=none
    terraform state list
    module.events.aws_cloudwatch_event_rule.rule
    module.events.aws_cloudwatch_event_target.target
    module.events.aws_lambda_permission.permission
    module.iam.data.aws_iam_policy_document.policy_doc
    module.iam.data.aws_iam_policy_document.role_doc
    module.iam.aws_iam_policy.policy
    module.iam.aws_iam_role.role
    module.iam.aws_iam_role_policy_attachment.attachment
    module.lambda.aws_lambda_function.func
    ```
    workspace にリソースがない場合は
    ```bash frame=none
...
<LinkPreview url="https://zenn.dev/machamp/articles/modify-terraform-resource-name" />
